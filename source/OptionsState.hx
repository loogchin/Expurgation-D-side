...
class PreferencesSubstate extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		showCharacter = new Character(840, 170, 'bf-testing', true);
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Sussy Mode':
						ClientPrefs.SussyMode = !ClientPrefs.SussyMode;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Sussy Mode':
				daText = "Expurgation but more StopSign.";
			case 'Downscroll':
