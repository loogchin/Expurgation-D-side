...
class TrickyButton extends FlxSprite
{
    ...
    public function new(_x:Int,_y:Int,pngOne:String,pngTwo:String, func:Void->Void, pogn:String = 'button', tweenDistanceX:Float = 0, tweenDistanceY:Float = 0)
    {
        ...
        spriteOne = new FlxSprite(trueX + tweenX, trueY + tweenY).loadGraphic(Paths.image(pngOne,"shared"));
        spriteTwo = new FlxSprite(trueX + tweenX, trueY + tweenY).loadGraphic(Paths.image(pngTwo,"shared"));
    ...
    public function highlight(playSound:Bool = true)
    {
        if (playSound)
        {
            var sound:FlxSound = new FlxSound().loadEmbedded(Paths.sound("Hover","shared"));
            sound.play();
        }
        spriteTwo.alpha = 1;
        spriteOne.alpha = 0;
    ...
    {
        var sound:FlxSound = new FlxSound().loadEmbedded(Paths.sound("confirm","shared"));
        sound.play();
